...
class App extends Component {
  ...
  // Fetch our Blockchain data
  async loadBlockchainData() {
    const web3 = window.web3;
    const accounts = await web3.eth.getAccounts();
    this.setState({ account: accounts[0] });

    // Fetch our smarts contracts from Blockchain
  ...
  // Connect our React App with Blockchain usinig web3js
  async loadWeb3() {
    if (window.ethereum) {
      ...
      window.alert(
        "Non-Ethereum browser detected. You shloud consider trying MetaMask!"
      );
    }
  }
